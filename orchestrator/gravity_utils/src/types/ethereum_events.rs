...
impl ValsetUpdatedEvent {
    ...
    pub fn from_log(input: &Log) -> Result<ValsetUpdatedEvent, GravityError> {
        ...
        let valset_nonce: u64 = match valset_nonce.to_string().parse() {
            Ok(nonce) => nonce,
            Err(err) => {
                return Err(GravityError::InvalidEventLogError(format!(
                    "Failed to parse the valset nonce: {:?}",
                    err
                )));
            }
        };
        ...
        let event_nonce: u64 = match event_nonce.to_string().parse() {
            Ok(nonce) => nonce,
            Err(err) => {
                return Err(GravityError::InvalidEventLogError(format!(
                    "Failed to parse the event nonce while downcasting from Uint256 -> u64: {:?}",
                    err
                )));
            }
        };
        ...
        let len_eth_addresses: usize = match len_eth_addresses.to_string().parse() {
            Ok(addr) => addr,
            Err(err) => {
                return Err(GravityError::InvalidEventLogError(
                    format!(
                        "Failed to parse eth address array length while downcasting Uint256 -> usize: {:?}",
                        err
                    )
                ));
            }
        ...
        // Leaving this for now because its out of scope, but it should be len_powers.to_string()... etc.
        let len_powers: usize = match len_eth_addresses.to_string().parse() {
            Ok(len) => len,
            Err(err) => {
                return Err(GravityError::InvalidEventLogError(format!(
                    "Failed to parse powers array length while downcasting Uint256 -> usize: {:?}",
                    err
                )));
            }
        };
        ...
        for i in 0..len_eth_addresses {
            ...
            let power: u64 = match power.to_string().parse() {
                Ok(power) => power,
                Err(err) => {
                    return Err(GravityError::InvalidEventLogError(format!(
                        "Failed to parse power while downcasting Uint256 -> u64: {:?}",
                        err
                    )));
                }
            };
